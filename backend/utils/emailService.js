...
/**
 * Sends a premium password reset OTP email using Brevo's HTTPS REST API.
 * @param {string} toEmail - The recipient's email address.
 * @param {string} otpCode - The 6-digit verification code.
 * @returns {Promise<boolean>}
 */
exports.sendPasswordResetOTPEmail = async (toEmail, otpCode) => {
  const apiKey = process.env.BREVO_API_KEY;
  const emailUser = 'mmm04opensooq@gmail.com'; // Your verified Brevo sender email
  ...
  if (!apiKey) {
    console.warn('[Brevo Debug] Warning: BREVO_API_KEY is not defined in environment variables.');
    return triggerBypass(toEmail, otpCode, 'BREVO_API_KEY is missing', true);
  }
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Reset Your PathFinder AI Password</title>
      <style>
        body {
          margin: 0;
          padding: 0;
  ...
  try {
    console.log(`[Brevo Debug] Attempting to dispatch password reset OTP email to ${toEmail} via HTTPS API...`);
    ...
    const response = await axios.post(
      'https://api.brevo.com/v3/smtp/email',
      {
        ...
        subject: `${otpCode} is your PathFinder AI password reset code`,
        htmlContent: htmlContent,
      },
      {
        headers: {
    ...
    if (response.status === 200 || response.status === 201 || response.status === 202) {
      console.log(`[Brevo Debug] Successfully sent password reset OTP email to ${toEmail} via HTTPS (ID: ${response.data.messageId})`);
      return true;
    } else {
      console.error('[Brevo Debug] Unexpected response status from Brevo API:', response.status, response.data);
      throw new Error('Brevo API returned unexpected status.');
    ...
    const errorDetails = error.response?.data?.message || error.message || '';
    console.error('[Brevo Debug] Error sending password reset email through Brevo API:', errorDetails);
    
    const allowBypass = process.env.ALLOW_OTP_BYPASS !== 'false';
    if (allowBypass) {
      return triggerBypass(toEmail, otpCode, errorDetails, true);
    }
    throw new Error(errorDetails || 'Failed to dispatch email via Brevo API.');
  }
};
...
function triggerBypass(toEmail, otpCode, reason, isPasswordReset = false) {
  console.warn('\n==================================================');
  ...
  if (isPasswordReset) {
    console.warn('Paste it in the forgot password screen to complete the reset.');
  } else {
