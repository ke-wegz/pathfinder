...
import {
    Github, AlertCircle, CheckCircle, Compass, Shield,
    UserPlus, User, Mail, Lock, Eye, EyeOff, ArrowRight
} from 'lucide-react';
...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Dispatches OTP to email before creating Firebase Auth profile
            await api.post('/users/otp/send', { email });
            setSuccess(t('Verification code sent successfully to your email!'));
            setStep(2);
            setResendTimer(60);
        } catch (err) {
    ...
    const handleVerifyOTP = async (e) => {
        ...
        try {
            // 1. Verify OTP code with backend first
            await api.post('/users/otp/verify', { email, otp });

            // 2. Proceed with authenticating and building backend profile
    ...
    const handleSocialSignup = (provider) => {
        alert(t(`${provider} signup coming soon!`));
    };
            ...
            <div className="max-w-md w-full">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                        {step === 1 
                            ? t('Start your journey to finding the perfect career path') 
                            : t('Enter the code we sent to complete your registration')
                        }
                    </p>
                </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <label htmlFor="terms" className="text-sm text-gray-600 dark:text-gray-400">
                                        {t('I agree to the')}{' '}
                                        <a href="#" className="text-primary-600 hover:text-primary-700 font-medium">{t('Terms of Service')}</a>
                                        {' '}{t('and')}{' '}
                                        <a href="#" className="text-primary-600 hover:text-primary-700 font-medium">{t('Privacy Policy')}</a>
                                    </label>
                                </div>
